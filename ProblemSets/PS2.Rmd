...
In this section, we will manually compute sample mean, denoted by $\hat{\mu}$, compute the standard error of $\hat{\mu}$, and calculate the $t$-statistic to conduct a hypothesis test at the 5$\%$ significance level. We then will compare our computations with the intercept-only regression result, a linear regression model with only an intercept. 
