...
library(pacman)
p_load(ggthemes, viridis, knitr, extrafont, tidyverse, magrittr, latex2exp, parallel, Ecdat, wooldridge, dslabs, ggforce, svglite, grDevices)
# Define colors
red_pink <- "#e64173"
met_slate <- "#23373b" # metropolis font color
# Notes directory
dir_slides <- "~/GitHub/EC320_Econometrics/Lectures/02-Statistics_Review/"
# Knitr options
...
theme_simple <- theme_bw() + theme(
  ...
  text = element_text(family = "Arial", color = met_slate, size = 14),
...
- Lab today 
- Exercise 1 this Friday by 11:59 p.m. 
  ...
  - Please have your work knitted in html format.
- Problem Set 1 will be posted by the end of this week, which will be due next Friday 11:59 p.m.

Issues with .mono[R]?

- Lab today
- I have office hours today after class (15:00-16:00).
...
```{R, unif2, echo = F,  fig.height = 3.75}
x <- seq(-1, 3, 0.01)
y <- seq(1, 1.5, 0.01)
z <- seq(0, 2 ,0.01)
...
```{R, norm2, echo = F,  fig.height = 3.75}
df <- tibble(x = seq(-4, 4, 0.01), y = dnorm(x)) %>%
  rbind(., tibble(x = seq(4, -4, -0.01), y = 0))
ggplot() +
  scale_x_continuous(limits = c(-4, 4), expand=c(0,0)) +
