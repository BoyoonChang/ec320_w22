...
What must we __assume__ to interpret $\hat{\beta}_1$ $=$ .hi-purple[`r round(b1, 2)`] as the return to schooling?
...
5. **Non-autocorrelation:** The values of error terms have independent of one another (*i.e.,* $E[u_i u_j]=0, \forall i \text{ s.t. } i \neq j$)
...
.footnote[
.pink[†] **Random Sampling:** We have a random sample from the population of interest.
]
...
5. **Non-Autocorrelation:** Any pair of error terms are drawn independently of each other (*i.e.,* $\mathop{\text{E}}(u_i u_j) = 0 \ \forall \ i \text{ s.t. } i \neq j$)
