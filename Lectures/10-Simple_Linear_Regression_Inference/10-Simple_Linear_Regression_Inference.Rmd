...
$$\hat{\beta} \sim \mathop{N}\left( \beta_1, \frac{\sigma^2}{\sum_{i=1}^n (X_i - \bar{X})^2} \right)$$
...
geom_histogram(data = line_df, aes(intercept), fill = red_pink, alpha = 0.75, bins = 40) +
  ...
  scale_x_continuous(breaks = lm0$coefficients[1], labels = TeX("$\\beta_0$")) +
...
geom_histogram(data = line_df, aes(slope), fill = red_pink, alpha = 0.75, bins = 40) +
  ...
  scale_x_continuous(breaks = lm0$coefficients[2], labels = TeX("$\\beta_1$")) +
  theme(axis.text.x = element_text(size = 50),
      axis.text.y = element_blank(),
      rect = element_blank(),
      axis.title.y = element_blank(),
...
**From our previous simulation:** `r ci_df$ci_contains %>% multiply_by(100) %>% mean() %>% round(1)`% of 95% confidence intervals contain the true parameter value of $\beta_1$.
